...
const getConcerts = async(req, res) => {
    try {        
        const allConcerts = await Concert.find().populate({path:'hall', model:'hall'}).populate({path:'artists', model:'artist'}).populate({path:'tickets', model:'ticket'});
...
const getConcertById = async(req, res) => {
    try {
        const {id} = req.params;
        const concert = await Concert.findById(id).populate({path:'hall', model:'hall'}).populate({path:'artists', model:'artist'}).populate({path:'tickets', model:'ticket'});
